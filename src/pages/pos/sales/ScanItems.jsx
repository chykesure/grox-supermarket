...
function ScanItems() {
  ...
  // Pending Carts State & Modal Control
  ...
  const subtotal = cart.reduce((sum, item) => {
    ...
    if (item.type === "wholesale") {
      // Logic: (Packs * PackPrice) + (Leftovers * PiecePrice)
      const packTotal = (item.packCount || 0) * item.sellingPrice;
      const leftover = (item.quantity || 0) - ((item.packCount || 0) * (item.wholesalePack || 0));
      const piecePrice = item.sellingPricePerPiece || (item.sellingPrice / (item.wholesalePack || 1));
      const leftoverTotal = leftover > 0 ? leftover * piecePrice : 0;
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      // Only refocus if user is NOT already in an input field (search, cash, customer search, etc)
      // and not inside a SweetAlert modal
  ...
  const addProductToCart = (product, { quantity = 1, packCount = 0, type = "retail", sellingPriceOverride } = {}) => {
    ...
    const entry = {
      ...
      // Calculate per piece price if wholesale
      sellingPricePerPiece: product.sellingPricePerPiece || (product.wholesalePack ? product.sellingPrice / product.wholesalePack : product.sellingPrice),
    };
  ...
  const addToCartWithSweetAlert = async (product) => {
    ...
    // Check if wholesale is applicable and cheaper (user incentive)
    if (product.wholesalePack && product.wholesalePack > 0) {
        ...
        html: `
          <div class="grid grid-cols-2 gap-4 text-left mt-4">
            <div class="bg-gray-700 p-3 rounded border border-gray-600 cursor-pointer hover:border-emerald-500 transition" onclick="Swal.clickConfirm()">
              ...
              <div class="font-bold text-lg">${formatMoney(product.sellingPrice)} / pcs</div>
            </div>
            <div class="bg-gray-700 p-3 rounded border border-gray-600 cursor-pointer hover:border-emerald-500 transition" onclick="Swal.clickCancel()">
              ...
              <div class="font-bold text-lg text-emerald-400">${formatMoney(product.wholesalePrice)} / pack (${product.wholesalePack} pcs)</div>
            </div>
          </div>
      ...
      // Note: confirm is Retail (default behavior), cancel is Wholesale
      chooseWholesale = result.isDismissed;
    }

    if (chooseWholesale) {
      const { value: packInput } = await darkSwal.fire({
        title: "Enter Pack Quantity",
        input: "number",
        inputLabel: `Pack size: ${product.wholesalePack} pcs`,
        inputValue: 1,
        showCancelButton: true,
        inputAttributes: { min: 1, max: Math.floor(product.stock / product.wholesalePack) },
      });
      ...
      const { value: pcs } = await darkSwal.fire({
        ...
        input: "number",
        inputLabel: `Available: ${product.stock} pcs`,
        inputValue: 1,
        showCancelButton: true,
  ...
  const suspendCart = () => {
    ...
    const newPending = {
      ...
      items: cart,
      subtotal,
      customer: selectedCustomer, // Save customer with cart
      createdAt: Date.now()
    };
      ...
      items: cart.map(item => {
        if (item.type === "wholesale") {
          ...
          return {
            ...
            subtotal: (item.packCount || 0) * item.sellingPrice + (leftover > 0 ? leftover * piecePrice : 0),
            type: "wholesale",
    ...
    <div className="flex flex-col bg-slate-900 text-slate-200 h-screen font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex justify-between items-end text-xs text-slate-400">
                  <div className="flex flex-col gap-1">
                    ...
                    {product.wholesalePack > 0 && (
                      <span className="bg-blue-500/10 text-blue-400 px-1.5 py-0.5 rounded inline-block w-max">
                        Pack: {product.wholesalePack} ({formatMoney(product.wholesalePrice)})
                      </span>
                    )}
                  </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1 pr-2">
                        ...
                        <div className="flex items-center gap-2 text-xs mt-1">
                          ...
                          {item.type === 'wholesale' && (
                            <span className="text-slate-500">({item.packCount} packs + extras)</span>
                          )}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-emerald-400">
                          {(() => {
                            ...
                            if (item.type === "wholesale") {
                              ...
                              const leftover = (item.quantity || 0) - ((item.packCount || 0) * (item.wholesalePack || 0));
                              const piecePrice = item.sellingPricePerPiece || (item.sellingPrice / (item.wholesalePack || 1));
                              total = packTotal + (leftover > 0 ? leftover * piecePrice : 0);
                            } else {
                              total = (item.quantity || 0) * item.sellingPrice;
                            }
                        ...
                        <div className="text-xs text-slate-500">{formatMoney(item.sellingPrice)} each</div>
                      </div>
                    </div>
                      ...
                      <button
                        onClick={async () => {
                          const { value: quantityType } = await Swal.fire({
                            ...
                            inputOptions: {
                              retail: 'Retail (Pcs)',
                              wholesale: `Wholesale (Packs of ${item.wholesalePack})`,
                            },
                            background: '#1f2937',
                            color: '#131313',
        ...
        <div className="w-80 flex-shrink-0 bg-slate-800 border-l border-slate-700 flex flex-col shadow-xl">
          {/* 
    CHANGES:
    1. Removed 'mt-auto' from the button container (stops pushing buttons to bottom).
    2. Reduced padding to p-2 and gap to gap-2 for tighter layout.
  */}
