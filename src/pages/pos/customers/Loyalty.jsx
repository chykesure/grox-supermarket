...
function Loyalty() {
  ...
  useEffect(() => {
    ...
    const fetchLoyalty = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:8080/api/customers/${selectedCustomerId}/loyalty`);
  ...
  const handleAdjustPoints = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8080/api/customers/${selectedCustomerId}/loyalty/adjust`,
        payload
      );
                ...
                <form onSubmit={handleAdjustPoints} className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-lg font-medium transition ${adjustmentType === "add"
                    ...
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-lg font-medium transition ${adjustmentType === "redeem"
                      ...
                      selectedCustomer.history.map((entry) => (
                        <tr key={entry.id} className="hover:bg-gray-700/40 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <span
                              className={`px-2.5 py-1 rounded text-xs font-medium ${entry.action === "Earned" ? "bg-green-900/50 text-green-300" : "bg-yellow-900/50 text-yellow-300"
                                }`}
                            >
                              {entry.action}
                            </span>
