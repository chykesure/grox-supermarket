        ...
        <div className="space-y-4">
          ...
          <ul className="mt-2">
            {modules
              .filter((module) => {
                if (userRole === "cashier") {
                  return module.key === "sales";
                }
                return true;
              })
              .map((module) => {
                // Limit sales flows for cashier
                if (userRole === "cashier" && module.key === "sales") {
                  module.flows = module.flows.filter((flow) =>
                ...
                const parentActive =
                  module.key !== "sales"
                    ? module.flows.some(
                        (flow) => pathname === flow.path || pathname.startsWith(flow.path)
                      )
