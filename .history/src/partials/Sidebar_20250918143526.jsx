        ...
        <div className="space-y-4">
          ...
          <ul className="mt-2">
            {modules.map((module) => (
              <SidebarLinkGroup
                key={module.key}
                activecondition={pathname.includes(module.key)}
              >
                {(handleClick, open) => (
                  <>
                    <a
                      href="#0"
                      className={`flex items-center justify-between px-3 py-2 rounded-md transition ${(pathname.startsWith(`/${module.key}`) ||
                          module.flows.some(flow =>
                            pathname.includes(
                              `/${module.key}/${flow.title.replace(/\s+/g, "-").toLowerCase()}`
                            )
                          ))
                        ...
                        {module.flows.map((flow, i) => (
                          <li key={i}>
                            <NavLink
                              ...
                              to={`/${module.key}/${flow.title
                                .replace(/\s+/g, "-")
                                .toLowerCase()}`}
                              className={({ isActive }) =>
                                `flex items-center gap-2 px-2 py-1 rounded-md text-sm transition ${isActive
