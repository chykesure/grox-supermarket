          ...
          <ul className="mt-2">
            {modules.map((module) => {
              const isParentActive =
                pathname === `/${module.key}` || module.flows.some(flow => pathname === flow.path);

              return (
                <SidebarLinkGroup key={module.key} activecondition={isParentActive} initialOpen={isParentActive}>
                  {(handleClick, open) => (
                    <>
                      {/* Parent link */}
                      <a
                        ...
                        onClick={(e) => {
                          ...
                          if (!open) {
                            handleClick();
                          } else if (module.defaultPath && pathname === `/${module.key}`) {
