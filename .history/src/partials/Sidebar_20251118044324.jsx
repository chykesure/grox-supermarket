...
function Sidebar({ sidebarOpen, setSidebarOpen, variant = "default" }) {
  ...
  // Get user role
  const userRole = localStorage.getItem("role") || "guest"; // "cashier", "admin", etc.
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <ul className="mt-2">
            {modules
  .filter((module) => {
    // Only show Sales module for cashier
    if (userRole === "cashier") return module.key === "sales";
    return true;
  })
  .map((module) => {
    // Create a new flows array for cashier without mutating original
    const flows =
      userRole === "cashier" && module.key === "sales"
        ? module.flows.filter((flow) =>
            ["/sales/scan-items", "/sales/receipt"].includes(flow.path)
          )
        : module.flows;

    const parentActive =
      module.key !== "sales"
        ? flows.some((flow) => pathname === flow.path || pathname.startsWith(flow.path))
          ...
          <>
            ...
            <a
              ...
              onClick={(e) => {
                ...
                if (module.key === "sales") navigate("/sales/scan-items");
                else navigate(module.defaultPath);
              }}
            >
