...
const StockPage = () => {
  ...
  const handleCurrentItemChange = (field, value) => {
    ...
    updatedItem[field] = numericFields.includes(field) ? Number(value) : value;

    const cost = field === "costPrice" ? Number(value) : updatedItem.costPrice;
    const selling =
      field === "sellingPrice" ? Number(value) : updatedItem.sellingPrice;

    // ------------------------------------------
    // 🔥 INSTANT markup calculation (no blur, no click)
    // ------------------------------------------
    if (cost > 0 && selling > 0) {
    ...
    // ------------------------------------------
    // Projected balance updates instantly
    // ------------------------------------------
    updatedItem.projectedBalance =
      (updatedItem.currentStock || 0) + qty;
