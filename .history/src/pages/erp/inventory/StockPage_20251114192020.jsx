...
const StockPage = () => {
  ...
  const handleCurrentItemChange = (field, value) => {
    ...
    const wholesale = updated.wholesalePrice || 0;
    ...
    // 🔥 Recalculate markup instantly
    if (cost > 0 && selling > 0) {
      updated.markup = Number((((selling - cost) / cost) * 100).toFixed(2));
    }

    // 🔥 Recalculate selling instantly if markup is edited
    if (field === "markup" && cost > 0) {
      const newSelling = cost + (cost * updated.markup) / 100;
    ...
    // 🔥 Recalculate projected balance instantly
