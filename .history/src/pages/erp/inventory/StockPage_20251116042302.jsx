...
const StockPage = () => {
  ...
  const calculateDerived = (item) => {
    ...
    item.subtotal = Number((qty * (selling > 0 ? selling : cost)).toFixed(2));
  ...
  const handleEditField = (index, field, value) => {
    ...
    // Validation
    const errors = {};
    if (item.costPrice > item.sellingPrice) errors.cost = "Cost cannot exceed Selling Price";
    if (item.wholesalePrice > item.sellingPrice) errors.wholesale = "Wholesale cannot exceed Selling Price";
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody className="text-gray-200">
                      {cartItems.map((item, index) => {
                        const isEditing = editingIndex === index; // <-- use correct state
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-right font-semibold text-emerald-400">
                              {formatMoney(
                                isEditing ? editForm.quantity * editForm.costPrice : item.subtotal
                              )}
                            </td>
                            ...
                            <td className="px-4 py-3 text-center flex items-center gap-3 justify-center">
                              {!isEditing ? (
                                <Edit3
                                  ...
                                  onClick={() => {
                                    setEditingIndex(index);
                                    setEditForm({ ...item }); // populate form with current item
                                  }}
                                />
                              ) : (
                                <>
                                  <Check
                                    ...
                                    onClick={() => {
                                      ...
                                      updatedCart[index] = { ...editForm, subtotal: editForm.quantity * editForm.costPrice };
