...
const StockPage = () => {
  ...
  const handleCurrentItemChange = (field, value) => {
    ...
    const updated = { ...currentItem, [field]: isNaN(num) ? value : num };

    const cost = updated.costPrice || 0;
    let selling = updated.sellingPrice || 0;
    let markup = updated.markup || 0;
    const qty = updated.quantity || 1;

    // Prevent wholesalePrice > sellingPrice
    if (field === "wholesalePrice" && num > selling) {
      Swal.fire({
        icon: "error",
        title: "Oops!",
    ...
    // If cost or selling price changes, recalc markup
    // 🔥 Recalculate markup instantly when cost or selling price changes
if (field === "costPrice" || field === "sellingPrice") {
  if (cost > 0 && selling > 0) {
    ...
    // If markup changes, recalc selling price
    if (field === "markup") {
      selling = cost + (cost * updated.markup) / 100;
      updated.sellingPrice = Number(selling.toFixed(2));

      // Optional: Ensure wholesalePrice does not exceed new sellingPrice
      if (updated.wholesalePrice > selling) updated.wholesalePrice = selling;
    }

    // Recalculate subtotal based on selling price
    updated.subtotal = qty * selling;
  ...
  const addToCart = () => {
    ...
    setCurrentItem({
      ...
      sellingPrice: 0,
      markup: 0,
      wholesalePrice: 0,      // 👌 reset wholesale price
      wholesalePackSize: 1,   // 👌 reset wholesale pack size (correct field)
      subtotal: 0,
      currentStock: 0,
      projectedBalance: 0,
    });
                    ...
                    <tbody className="text-gray-200">
                      {cartItems.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right">{formatMoney(item.costPrice)}</td>
                          <td className="px-4 py-3 text-right">{formatMoney(item.sellingPrice)}</td>
                          <td className="px-4 py-3 text-right">{formatMoney(item.wholesalePrice)}</td>
                          <td className="px-4 py-3 text-center">{item.wholesalePackSize}</td>
                          <td className="px-4 py-3 text-center">{item.markup}%</td>
