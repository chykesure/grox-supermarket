...
function StockIn() {
  ...
  const addProductToStockIn = (product) => {
    if (!stockInItems.find((item) => item.id === product.id)) {
      setStockInItems([...stockInItems, { ...product, qty: 1, discount: 0, vat: 0, highlighted: false }]);
    }
  };
  ...
  const confirmStockIn = () => {
    ...
    setStockHistory([
      ...stockHistory,
      {
        ...
        invoiceNumber,
        invoiceDate,
        items: stockInItems.map(item => ({ ...item, highlighted: false })), // remove highlight
        grandTotal,
      },
    ]);
        ...
        <table className="w-full table-auto border-collapse">
          ...
          <tbody className="divide-y divide-gray-700">
            {filteredProducts.map((p) => {
              const stockItem = stockInItems.find((item) => item.id === p.id) || { qty: 0, discount: 0, vat: 0, highlighted: false };
              return (
                <tr
                  key={p.id}
                  className={`hover:bg-gray-700 ${stockItem.highlighted ? 'bg-yellow-600' : ''}`}
                >
            ...
            <tbody className="divide-y divide-gray-700">
              {stockHistory.map((h, idx) => (
                <tr
                  ...
                  onClick={() => {
                    const restoredItems = h.items.map(item => ({ ...item, highlighted: true }));

                    setSelectedSupplier(suppliers.find((s) => s.name === h.supplier)?.id || "");
                    setInvoiceNumber(h.invoiceNumber);
                    setInvoiceDate(h.invoiceDate);
                    setStockInItems(restoredItems);

                    // Remove from history to prevent duplicates
                    setStockHistory(stockHistory.filter((_, i) => i !== idx));

                    showToast("success", "Stock-In restored for editing.");
                  }}
                >
