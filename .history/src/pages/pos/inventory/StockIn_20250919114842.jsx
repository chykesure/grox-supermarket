            ...
            {filteredProducts.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-1 text-center">{calculateTotal(stockItem || { price: p.price, qty: 0, discount: 0, vat: 0 })}</td>
                  <td className="px-2 py-1 text-center flex justify-center gap-1">
                    <button
                      onClick={() =>
                        isAdded
                          ? removeStockInItem(p.id)
                          : addProductToStockIn(p)
                      }
                      className={`p-1 rounded ${isAdded ? "bg-green-500 hover:bg-green-600 text-white" : "bg-blue-500 hover:bg-blue-600 text-white"}`}
                    >
                      {isAdded ? "Added" : "Add"}
                    </button>
                    {isAdded && (
              ...
              {stockHistory.map((h, idx) => (
                <tr
                  ...
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      setStockInItems(prev => prev.map(item => ({ ...item, highlighted: false })));
                    }, 4000);
                  }}
                >
