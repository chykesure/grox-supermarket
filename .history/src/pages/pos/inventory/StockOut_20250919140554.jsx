...
function StockOut() {
  ...
  const grandTotal = stockOutItems.reduce(
  ...
  const confirmStockOut = () => {
    if (!requester || stockOutItems.length === 0) {
      setMessage({
        type: "error",
        text: "Please enter requester and select at least one product.",
      });
      return;
    }
    ...
    for (const item of stockOutItems) {
      ...
      if (item.qty > product.stock) {
        setMessage({
          type: "error",
          text: `Not enough stock for ${item.name}. Available: ${product.stock}`,
        });
        return;
      }
    }
      ...
      <div className="bg-gray-800 p-4 rounded-lg shadow-md flex flex-col md:flex-row gap-4">
        <div className="flex-1">
          ...
          <input
            ...
            className="mt-1 p-2 rounded-md w-full bg-gray-50 border border-gray-300 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400"
              ...
              <tr key={p.id} className="hover:bg-gray-700">
                ...
                <td className="px-2 py-1 text-center">
                  <input
                    ...
                    className="w-20 p-2 rounded-md bg-gray-50 border border-gray-300 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 text-center"
                  />
                </td>
                ...
                <td className="px-2 py-1 text-center">
                  ₦
                  {calculateTotal(
                    stockOutItems.find((i) => i.id === p.id) || {
                      price: p.price,
                      qty: 0,
                    }
                  )}
                ...
                <td className="px-2 py-1 text-center">
                  <button
                    onClick={() =>
                      setStockOutItems(stockOutItems.filter((i) => i.id !== p.id))
                    }
