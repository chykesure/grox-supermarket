...
function RevenueReport() {
  ...
  // -----------------------------
  // Filter returns for a given row & date
  // -----------------------------
  const getFilteredReturns = (row) => {
    return returnData.filter((r) => {
      ...
      if (startDate && endDate) {
        ...
        return rDate >= new Date(startDate);
      } else if (endDate) {
        const end = new Date(endDate);
        end.setHours(23, 59, 59, 999);
        return rDate <= end;
  ...
  // -----------------------------
  // Compute filtered revenue with returns
  // -----------------------------
  const filteredRevenueData = revenueData.map((row) => {
    const periodReturns = getFilteredReturns(row);
    ...
    const netSales = row.sales - totalReturns;
    const netProfit = row.profit - totalReturns;

    return { ...row, returns: totalReturns, netSales, netProfit };
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-gray-800 p-4 rounded-2xl shadow"><p className="text-gray-400 text-sm">Total Sales</p><p className="text-xl font-semibold text-gray-200">{formatCurrency(totals.sales)}</p></div>
        <div className="bg-gray-800 p-4 rounded-2xl shadow"><p className="text-gray-400 text-sm">Total Cost</p><p className="text-xl font-semibold text-red-400">{formatCurrency(totals.cost)}</p></div>
        <div className="bg-gray-800 p-4 rounded-2xl shadow"><p className="text-gray-400 text-sm">Total Profit</p><p className="text-xl font-semibold text-green-400">{formatCurrency(totals.profit)}</p></div>
        <div className="bg-gray-800 p-4 rounded-2xl shadow"><p className="text-gray-400 text-sm">Total Returns</p><p className="text-xl font-semibold text-yellow-400">{formatCurrency(totals.returns)}</p></div>
      </div>
