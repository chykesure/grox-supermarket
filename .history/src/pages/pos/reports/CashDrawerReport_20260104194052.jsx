...
function ReturnItemsHistory() {
  ...
  const totals = filteredData.reduce(
    (acc, row) => {
      const rowItemCount = row.items?.reduce((sum, item) => sum + (item.quantity || 0), 0) || 0;
      const rowRefund = parseFloat(row.totalRefund) || 0;
      acc.totalRefunds += rowRefund;
      acc.itemCount += rowItemCount;
      return acc;
    },
  ...
  const averageRefund =
    filteredData.length > 0 ? totals.totalRefunds / filteredData.length : 0;
  ...
  const exportToExcel = () => {
    const data = filteredData.map((row) => ({
      ...
      "Product(s)": row.items
        ...
        .join("; "),
      Quantity: row.items.reduce((sum, i) => sum + (i.quantity || 0), 0),
      "Refund Amount (₦)": formatCurrency(row.totalRefund || 0),
