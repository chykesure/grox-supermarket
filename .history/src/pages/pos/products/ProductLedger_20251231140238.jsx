...
function ProductLedger() {
  ...
  const [ledgerData, setLedgerData] = useState(null); // Now holds { product, currentStock, ledger }
    ...
    const fetchLedger = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE}/${encodeURIComponent(selectedProduct)}${selectedSupplier ? `?supplierId=${selectedSupplier}` : ""
          }`
        );
        // res.data is now: { product, currentStock, ledger: [...] }
  ...
  if (!ledgerData) {
    var ledger = [];
    var productName = selectedProduct || "—";
    var currentStock = 0;
  } else {
    var { product: productName, currentStock, ledger } = ledgerData;
  }

  // Client-side filtering (supplier already filtered on backend optionally, but we allow extra date filter)
  ...
  // Recalculate running balance on filtered data
  ...
  // Get unique suppliers from the full ledger (for dropdown)
      ...
      <div className="mb-4 w-1/3">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setSelectedSupplier(""); // Reset supplier when product changes
          }}
