...
function AddProduct() {
  ...
  const [hasExpiry, setHasExpiry] = useState(false); // ✅ checkbox control state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setHasExpiry(false); // reset checkbox
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-gray-900 p-6 rounded-lg shadow-md text-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Row 2: Cost Price + Selling Price + Markup */}
        ...
        {/* Row 3: Category + Expiry Date (optional toggle) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              disabled={!hasExpiry}
              className={`w-full px-3 py-2 rounded-md border border-gray-700 ${hasExpiry ? "bg-gray-800 text-gray-200" : "bg-gray-700 text-gray-500 cursor-not-allowed"
                }`}
            />
          </div>
        </div>
