...
function EditProduct() {
  ...
  const [productData, setProductData] = useState({
    ...
    category: "",
    costPrice: "",
    markup: 1, // shown but not editable
    sellingPrice: "",
    minQty: "",
    maxQty: "",
    expiryDate: "",
  ...
  // helper to compute markup given cost and selling
  ...
  // handle input changes (two-way: cost/selling -> markup; name->sku)
  const handleChange = (e) => {
    ...
    setProductData((prev) => {
      ...
      // If user edits costPrice: recalc markup (if sellingPrice exists)
      if (name === "costPrice") {
        updated.markup = computeMarkup(value, prev.sellingPrice) || prev.markup;
        // optionally update sellingPrice if you prefer cost+markup => selling,
        // but requirement was markup auto-calc when entering selling price, so we keep selling unchanged.
      }

      // If user edits sellingPrice: recalc markup from costPrice
      if (name === "sellingPrice") {
        updated.markup = computeMarkup(prev.costPrice, value);
      }

      // Generate SKU if name changed and sku is empty
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ensure markup field is always a number or omitted
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4">
          <button
            ...
            className={`w-full md:w-auto px-6 py-2 rounded-md text-white transition flex items-center justify-center gap-2 ${isSubmitting ? "bg-gray-600 cursor-not-allowed" : "bg-green-600 hover:bg-green-500"
              }`}
          >
            {isSubmitting ? (
              <>
