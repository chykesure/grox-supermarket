...
import { cilUser, cilSave, cilPencil, cilTrash, cilCheckCircle, cilFilter } from "@coreui/icons";
...
function Login() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      else console.error("Fetch users failed:", data.message);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleAddUser = async (e) => {
    ...
    if (!email || role === "Select Role" || !status) {
      Swal.fire({ icon: "warning", title: "Incomplete Data", text: "Please fill all fields correctly" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        Swal.fire({ icon: "error", title: "Error", text: data.message || "Failed to add user" });
      } else {
        Swal.fire({ icon: "success", title: "Success", text: "User added successfully", timer: 2000, showConfirmButton: false });
        setEmail(""); setRole("Select Role"); setStatus("Active");
        fetchUsers();
      }
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        if (res.ok) {
          Swal.fire({ icon: "success", title: "Deleted", text: "User deleted successfully", timer: 2000, showConfirmButton: false });
          fetchUsers();
        }
      } catch (err) {
        console.error(err);
  ...
  const filteredUsers = users.filter(
    (u) =>
      ...
      (filterRole ? (u.role?.name === filterRole) : true)
  );
  ...
  return (
    <div className="w-full bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6 space-y-8 text-gray-100">
      ...
      <div className="w-full bg-gray-800 shadow-xl rounded-2xl border border-gray-700 overflow-hidden">
        ...
        <div className="p-6">
          <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
            <div>
            ...
            <div className="col-span-1 md:col-span-3 text-right">
              <button type="submit" disabled={loading}
                ...
                <CIcon icon={cilSave} className="w-4 h-4" /> {loading ? "Saving..." : "Save User"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 py-4 bg-gray-700/60 border-b border-gray-700 gap-3">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            <input
              ...
              className="px-3 py-2 w-full md:w-auto bg-gray-700 border border-gray-600 rounded-lg focus:outline-none text-gray-100 placeholder-gray-400"
            />
            ...
            <button className="bg-gray-600 hover:bg-gray-500 text-gray-100 font-medium px-4 py-2 rounded-lg inline-flex items-center gap-2 w-full md:w-auto transition" onClick={() => fetchUsers()}>
              <CIcon icon={cilFilter} className="w-4 h-4" /> Filter
            </button>
          </div>
        </div>
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user._id} className="border-t border-gray-700 hover:bg-gray-700/50 transition">
                  ...
                  <td className="px-6 py-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${user.status === "Active" ? "bg-green-600/20 text-green-400" : "bg-red-600/20 text-red-400"}`}>
                      {user.status}
                    </span>
                  </td>
                  <td className="px-6 py-3 flex justify-center gap-2">
                    ...
                    <button onClick={() => handleDelete(user._id)} className="p-2 bg-red-600/20 hover:bg-red-600/40 rounded-lg text-red-400">
