...
function ScanItems() {
  ...
  const [cashierName, setCashierName] = useState("Walk-in");
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setCashierName(parsed.name || parsed.username || "Walk-in");
    }
  }, []);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // ✅ Redirect to receipt page with invoice number
      setTimeout(() => {
  ...
  return (
    <div className="flex flex-col bg-gray-900 text-gray-100 h-screen">
      ...
      <div className="flex justify-between items-center px-6 py-4 border-b border-gray-700 bg-gray-800 shadow-sm">
        ...
        <div>Cashier: <span className="text-yellow-400">Walk-in</span></div>
