...
function SalesMgt() {
  ...
  useEffect(() => {
    if (!subpage) navigate("/sales/scan-items", { replace: true });
  }, [subpage, navigate]);

  const activeTab = subpage || "scan-items";

  const handleTabChange = (tab) => navigate(`/sales/${tab}`);

  const renderTabContent = () => {
    switch (activeTab) {
      case "scan-items": return <ScanItems />;
      case "adjust-qty": return <AdjustQty />;
      case "coupons": return <Coupons />;
      case "payments": return <Payments />;
      case "receipt": return <Receipt />;
      default: return <ScanItems />;
    }
  };
        ...
        <main className="grow">
          <div className="px-4 sm:px-6 lg:px-8 py-6 w-full max-w-7xl mx-auto">
            <div className="mb-6">
              ...
              <p className="text-gray-400 mt-1">Manage sales, adjust quantities, coupons, payments, and receipts.</p>
            </div>

            {/* Tabs */}
            <div className="mb-6 border-b border-gray-700">
              <nav className="flex space-x-8 -mb-px">
                {["scan-items","adjust-qty","coupons","payments","receipt"].map(tab => (
                  <button
                    key={tab}
