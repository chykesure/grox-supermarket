...
function ScanItems() {
  ...
  const handleCheckout = async () => {
    ...
    // Payload only sends productId, quantity, and price
    const payload = {
      invoiceNumber: `INV-${Date.now()}`, // generate invoice
      total: subtotal,                    // total cart amount
      paymentMode,
      items: cart.map(item => ({
        productId: item._id,
        quantity: item.quantity,
        price: item.sellingPrice,
        subtotal: item.sellingPrice * item.quantity, // subtotal per item
      })),
    };
    ...
    try {
      ...
      toast.success(`✅ Sale recorded successfully! Invoice: ${res.data.invoiceNumber}`);
      setCart([]);
      setPaymentMode("Cash");
      setCashTendered(0);
            ...
            {paymentMode === "Cash" && (
              <>
                ...
                <input
                  ...
                  onChange={(e) => {
                    // Remove non-numeric characters to get number
