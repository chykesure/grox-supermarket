...
function SalesMgt() {
  ...
  // Sidebar visibility
  const showSidebar = userRole !== "cashier";

  // Redirect to /sales/scan-items if no subpage or if user is cashier
  useEffect(() => {
    if (!subpage || userRole === "cashier") {
      navigate("/sales/scan-items", { replace: true });
    }
  }, [subpage, userRole, navigate]);
  ...
  return (
    <div className="flex overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-gray-200">
      ...
      {/* Main wrapper */}
      <div className={`relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden ${!showSidebar ? "ml-0" : ""}`}>
        {/* Header */}
        <Header />
