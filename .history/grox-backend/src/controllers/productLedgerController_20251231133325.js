...
// Get combined Product Ledger by productName (FIFO batch-aware)
export const getProductLedger = async (req, res) => {
  try {
    ...
    // 1️⃣ Stock-In entries
    const stockEntries = await Stock.find({ productId })
      .populate("supplierId", "name")
      .sort({ date: 1 });

    // Track remaining quantities per batch
    ...
    for (let batch of stockBatches) {
      ledger.push({
        ...
        supplierName: batch.supplierName,
        cashier: "—",
        quantityIn: batch.quantityRemaining + 0, // just show original
        quantityOut: 0,
        costPrice: batch.costPrice,
        sellingPrice: batch.sellingPrice,
        note: batch.note,
