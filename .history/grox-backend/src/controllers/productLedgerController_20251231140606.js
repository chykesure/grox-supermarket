import Stock from "../models/Stock.js"; // <- use Stock instead of Ledger
...
export const getProductLedger = async (req, res) => {
  try {
    ...
    // 1️⃣ Stock-In from Stocks collection
    const stockEntries = await Stock.find({ productId })
      .populate("supplierId", "name")
      .sort({ date: 1 });

    const formattedStockIns = stockEntries.map(entry => ({
      date: entry.date,
      type: "IN",
      referenceNumber: entry.referenceNumber || "—",
      supplierName: entry.supplierId?.name || "Unknown", // fallback if supplierId is missing
      cashier: "—",
      quantityIn: entry.quantity || 0,
      quantityOut: 0,
