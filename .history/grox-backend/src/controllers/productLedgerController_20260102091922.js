...
import Ledger from "../models/Ledger.js"; // <-- new ledger collection
...
export const getProductLedger = async (req, res) => {
  try {
    ...
    // 1. Fetch Stock-In and Returns from Ledger
    const ledgerQuery = { productId };
    if (supplierId) ledgerQuery.supplierId = new mongoose.Types.ObjectId(supplierId);

    const ledgerEntries = await Ledger.find(ledgerQuery)
      .sort({ date: 1 })
      .populate("supplierId", "name");

    // 2. Prepare FIFO batches for sales
    const fifoBatches = ledgerEntries
      .filter(entry => entry.type === "stock-in" || entry.type === "return")
      .map(entry => ({
    ...
    // 3. Map Stock-In & Returns to ledger rows
    let ledger = ledgerEntries.map(entry => ({
      date: entry.date,
      type: entry.type === "stock-in" ? "IN" : "IN",
      referenceNumber: entry.referenceNumber || entry._id.toString(),
      supplierName: entry.supplierId?.name || (entry.type === "return" ? "—" : "Unknown"),
      cashier: entry.type === "return" ? "—" : "—",
    ...
    // 6. Sort ledger and calculate running balance
