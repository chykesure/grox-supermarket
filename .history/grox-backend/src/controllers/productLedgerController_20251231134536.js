...
export const getProductLedger = async (req, res) => {
  try {
    ...
    // 1️⃣ Stock entries (oldest first), optional supplier filter
    const stockQuery = { productId: mongoose.Types.ObjectId(productId) };
    if (supplierId) stockQuery.supplierId = mongoose.Types.ObjectId(supplierId);
