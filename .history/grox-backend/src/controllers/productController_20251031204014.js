...
export const addProduct = async (req, res) => {
    try {
        ...
        // If req.body is empty or invalid
        if (!name || !sku)
        ...
        const sellingPriceCalculated = costPrice + (costPrice * (markup || 1) / 100);
        ...
        const product = new Product({
            ...
            costPrice,
            markup,
            sellingPrice: sellingPrice || sellingPriceCalculated, // calculated if missing
        });
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const sellingPriceCalculated = req.body.costPrice + (req.body.costPrice * (req.body.markup || 1) / 100);
...
export const addDiscount = async (req, res) => {
    try {
        ...
        const newDiscount = new Discount({ productId, discount, startDate, endDate });
...
export const getDiscounts = async (req, res) => {
    try {
        const discounts = await Discount.find().populate("productId", "name sku price");
        res.json(discounts);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
