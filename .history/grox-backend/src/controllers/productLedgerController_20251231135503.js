...
export const getProductLedger = async (req, res) => {
  try {
    ...
    // IN rows for display (original stock)
    ...
    // Internal stock batches for FIFO sales consumption
    ...
    // Add OUT rows for each sale item
    ...
    const returnEntries = await SalesReturn.find({
      "items.productId": productId
    }).sort({ date: 1 });
