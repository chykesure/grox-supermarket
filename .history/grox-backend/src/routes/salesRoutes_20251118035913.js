...
const getNextInvoiceNumber = async () => {
  ...
  return counter.seq; // plain sequential number: 1, 2, 3...
};
...
router.post("/", async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (!stock) {
        return res.status(400).json({ message: `Stock not found for product ${item.productId}` });
      }
    ...
    res.status(201).json({
      message: "Sale recorded, stock deducted, and ledger updated",
      saleId: newSale._id,
...
// Get sale by invoice number and populate product names
